...
function fmtPreco(v: number) {
  if (v >= 1_000_000) {
    const n = v / 1_000_000;
    return `R$${n % 1 === 0 ? n : n.toFixed(1)}M`;
  }
...
/* ─── Cabeçalho de seção (sem acordeão — sempre aberto) ─────────────────── */
function SectionTitle({ title }: { title: string }) {
  return (
    <p style={{ fontSize: 15, fontWeight: 700, color: '#111827', margin: '18px 0 10px' }}>
      {title}
    </p>
  );
}
