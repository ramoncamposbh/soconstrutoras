    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            <Home className="w-6 h-6 text-[#0E8F6E]" /> Imóveis Usados
          </h1>
          <p className="text-sm text-gray-500 mt-1">
            {imoveis.length}/{limite} imóveis cadastrados · imóveis recebidos em permuta
          </p>
        </div>
        <button
          onClick={() => abrirModal()}
      ...
      <div className="mb-6">
        ...
        <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
          <div
            className="h-full bg-[#0E8F6E] rounded-full transition-all"
            style={{ width: `${Math.min((imoveis.length / limite) * 100, 100)}%` }}
          />
        </div>
      </div>
