/**
 * cadastrar-rolim.js — Rolim (1 empreendimento — pasta plana)
 */
const fs   = require('fs');
const path = require('path');

const API   = 'https://soconstrutoras-production.up.railway.app/api/v1';
const EMAIL = 'rolim@soconstrutoras.com.br';
const SENHA = 'ROLIM@2026';
const BASE  = 'D:\\3 -IMOVEIS\\CONSTRUTORAS\\ATUAIS\\Rolim';
...
async function uploadImagens(TOKEN, empId, dir, facade) {
  ...
  // Pasta plana — só arquivos de imagem no nível raiz (ignora subdiretórios)
...
const EMPS = [
  {
    nome: 'Residencial Raul Rolim',
    dir:  BASE,  // pasta plana — imagens direto no root
    facade: 'Fachada.png',
    body: { tipo:'apartamento', status:'pronto', descricao:'Residencial Raul Rolim em BH. Empreendimento residencial entregue. Unidades esgotadas.',
...
async function main() {
  console.log('═══════════════════════════════════════════════════');
  console.log('  Rolim — 1 empreendimento');
  console.log('═══════════════════════════════════════════════════\n');
  let TOKEN;
  ...
  if (login.data?.access_token) { TOKEN = login.data.access_token; console.log('✅ Login OK'); }
    ...
    const reg = await api('/auth/register', { method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ email:EMAIL, password:SENHA, nome:'Rolim', razao_social:'Rolim Construtora e Incorporadora', role:'construtora' }) });
