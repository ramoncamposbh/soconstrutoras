/**
 * cadastrar-monterre.js — MONTERRE (3 empreendimentos)
 *   Arbo, Mont D'or, Santé — BH (vendidos)
 */
const fs   = require('fs');
const path = require('path');

const API   = 'https://soconstrutoras-production.up.railway.app/api/v1';
const EMAIL = 'monterre@soconstrutoras.com.br';
const SENHA = 'MONTERRE@2026';
const BASE  = 'D:\\3 -IMOVEIS\\CONSTRUTORAS\\ATUAIS\\MONTERRE';
...
const EMPS = [
  {
    nome: 'Arbo',
    dir:  path.join(BASE, '2026-08-19-Arbo'),
    facade: 'MONTERRE_ARBO_FACHADA_02.jpeg',
    body: {
      tipo:'apartamento', status:'pronto',
      descricao:'Arbo, empreendimento MONTERRE em Belo Horizonte. Apartamentos de alto padrão entregues com lazer exclusivo e acabamento diferenciado. Unidades esgotadas.',
  ...
  {
    nome: "Mont D'or",
    dir:  path.join(BASE, "2026-08-19-Mont D'or"),
    facade: '02_Fachada.jpeg',
    body: {
      tipo:'apartamento', status:'pronto',
      descricao:"Mont D'or, empreendimento MONTERRE em Belo Horizonte. Apartamentos de alto padrão entregues com área de lazer completa. Unidades esgotadas.",
      endereco:'Belo Horizonte', bairro:'Belo Horizonte', cidade:'Belo Horizonte', estado:'MG', cep:'30000-000',
      area_min:70, area_max:300, preco_min:null, preco_max:null, quartos_min:2, quartos_max:4, vagas:2,
    },
  },
  {
    nome: 'Santé',
    dir:  path.join(BASE, '2026-08-19-Santé'),
    facade: 'Area_de_servico.jpeg',
    body: {
      tipo:'apartamento', status:'pronto',
      descricao:'Santé, empreendimento MONTERRE em Belo Horizonte. Apartamentos de alto padrão entregues com lazer completo. Unidades esgotadas.',
      endereco:'Belo Horizonte', bairro:'Belo Horizonte', cidade:'Belo Horizonte', estado:'MG', cep:'30000-000',
      area_min:70, area_max:300, preco_min:null, preco_max:null, quartos_min:2, quartos_max:4, vagas:2,
    },
  },
...
async function main() {
  console.log('═══════════════════════════════════════════════════');
  console.log('  MONTERRE — 3 empreendimentos');
  console.log('═══════════════════════════════════════════════════\n');
  let TOKEN;
  ...
  if (login.data?.access_token) { TOKEN = login.data.access_token; console.log('✅ Login OK'); }
    ...
    const reg = await api('/auth/register', { method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ email:EMAIL, password:SENHA, nome:'MONTERRE', razao_social:'MONTERRE Empreendimentos Imobiliários', role:'construtora' }) });
