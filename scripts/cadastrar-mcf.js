/**
 * cadastrar-mcf.js — MCF (5 empreendimentos)
 *   Contour (Savassi), Lamon, Travessia, Unique, Vero — BH (vendidos)
 */
const fs   = require('fs');
const path = require('path');

const API   = 'https://soconstrutoras-production.up.railway.app/api/v1';
const EMAIL = 'mcf@soconstrutoras.com.br';
const SENHA = 'MCF@2026';
const BASE  = 'D:\\3 -IMOVEIS\\CONSTRUTORAS\\ATUAIS\\MCF';
...
const EMPS = [
  {
    nome: 'Contour',
    dir:  path.join(BASE, '2026-08-18-Contour'),
    facade: 'MCF_-_Edificio_Contour_Savassi_-_Areas_Comuns_-_Academia.jpeg',
    body: {
      tipo:'apartamento', status:'pronto',
      descricao:'Contour Savassi, empreendimento MCF no Savassi, Belo Horizonte. Apartamentos de alto padrão entregues com academia, espaço gourmet e hall social exclusivos. Unidades esgotadas.',
      endereco:'Savassi', bairro:'Savassi', cidade:'Belo Horizonte', estado:'MG', cep:'30140-110',
      area_min:70, area_max:300, preco_min:null, preco_max:null, quartos_min:2, quartos_max:4, vagas:2,
    },
  },
  {
    nome: 'Lamon',
    dir:  path.join(BASE, '2026-08-18-Lamon'),
    facade: '13-1.jpeg',
    body: {
      tipo:'apartamento', status:'pronto',
      descricao:'Lamon, empreendimento MCF em Belo Horizonte. Apartamentos de alto padrão entregues com acabamento superior. Unidades esgotadas.',
  ...
  {
    nome: 'Travessia',
    dir:  path.join(BASE, '2026-08-18-Travessia'),
    facade: 'Apartamento_tipo_-_final_01.jpeg',
    body: {
      tipo:'apartamento', status:'pronto',
      descricao:'Travessia, empreendimento MCF em Belo Horizonte. Apartamentos de alto padrão entregues com lazer exclusivo. Unidades esgotadas.',
      endereco:'Belo Horizonte', bairro:'Belo Horizonte', cidade:'Belo Horizonte', estado:'MG', cep:'30000-000',
      area_min:70, area_max:300, preco_min:null, preco_max:null, quartos_min:2, quartos_max:4, vagas:2,
    },
  },
  {
    nome: 'Unique',
    dir:  path.join(BASE, '2026-08-18-Unique'),
    facade: '01-2.jpeg',
    body: {
      tipo:'apartamento', status:'pronto',
      descricao:'Unique, empreendimento MCF em Belo Horizonte. Apartamentos de alto padrão entregues. Unidades esgotadas.',
      endereco:'Belo Horizonte', bairro:'Belo Horizonte', cidade:'Belo Horizonte', estado:'MG', cep:'30000-000',
      area_min:70, area_max:300, preco_min:null, preco_max:null, quartos_min:2, quartos_max:4, vagas:2,
    },
  },
  {
    nome: 'Vero',
    dir:  path.join(BASE, '2026-08-18-Vero'),
    facade: 'Fachada.jpeg',
    body: {
      tipo:'apartamento', status:'pronto',
      descricao:'Vero, empreendimento MCF em Belo Horizonte. Apartamentos de alto padrão entregues com acabamento e lazer superiores. Unidades esgotadas.',
      endereco:'Belo Horizonte', bairro:'Belo Horizonte', cidade:'Belo Horizonte', estado:'MG', cep:'30000-000',
      area_min:70, area_max:300, preco_min:null, preco_max:null, quartos_min:2, quartos_max:4, vagas:2,
    },
  },
...
async function main() {
  console.log('═══════════════════════════════════════════════════');
  console.log('  MCF — 5 empreendimentos');
  console.log('═══════════════════════════════════════════════════\n');
  let TOKEN;
  ...
  if (login.data?.access_token) { TOKEN = login.data.access_token; console.log('✅ Login OK'); }
    ...
    const reg = await api('/auth/register', { method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ email:EMAIL, password:SENHA, nome:'MCF', razao_social:'MCF Empreendimentos Imobiliários', role:'construtora' }) });
