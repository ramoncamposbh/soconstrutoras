/**
 * cadastrar-natusebelovale.js — NATUS E BELO VALE (2 empreendimentos)
 *   Espírito Santo Residencial — BH (vendido)
 *   Follow Savassi Residence   — Savassi, BH (vendido)
 */
const fs   = require('fs');
const path = require('path');

const API   = 'https://soconstrutoras-production.up.railway.app/api/v1';
const EMAIL = 'natusebelovale@soconstrutoras.com.br';
const SENHA = 'NATUSEBELOVALE@2026';
const BASE  = 'D:\\3 -IMOVEIS\\CONSTRUTORAS\\ATUAIS\\NATUS E BELO VALE';
...
const EMPS = [
  {
    nome: 'Espírito Santo Residencial',
    dir:  path.join(BASE, '2026-08-19-Espírito Santo Residencial'),
    facade: 'FACHADA_ESPIRITO_SANTO.jpeg',
    body: {
      tipo:'apartamento', status:'pronto',
      descricao:'Espírito Santo Residencial, empreendimento Natus e Belo Vale em Belo Horizonte. Apartamentos de alto padrão entregues com lazer completo. Unidades esgotadas.',
  ...
  {
    nome: 'Follow Savassi Residence',
    dir:  path.join(BASE, '2026-08-19-Follow Savassi Residence'),
    facade: 'fachada.jpeg',
    body: {
      tipo:'apartamento', status:'pronto',
      descricao:'Follow Savassi Residence, empreendimento Natus e Belo Vale no Savassi, Belo Horizonte. Apartamentos de alto padrão entregues em localização nobre. Unidades esgotadas.',
...
async function main() {
  console.log('═══════════════════════════════════════════════════');
  console.log('  NATUS E BELO VALE — 2 empreendimentos');
  console.log('═══════════════════════════════════════════════════\n');
  let TOKEN;
  ...
  if (login.data?.access_token) { TOKEN = login.data.access_token; console.log('✅ Login OK'); }
    ...
    const reg = await api('/auth/register', { method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ email:EMAIL, password:SENHA, nome:'Natus e Belo Vale', razao_social:'Natus e Belo Vale Empreendimentos', role:'construtora' }) });
