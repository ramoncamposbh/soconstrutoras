/**
 * cadastrar-rlcosta.js — RL COSTA (4 empreendimentos — todos vendidos)
 */
const fs   = require('fs');
const path = require('path');

const API   = 'https://soconstrutoras-production.up.railway.app/api/v1';
const EMAIL = 'rlcosta@soconstrutoras.com.br';
const SENHA = 'RLCOSTA@2026';
const BASE  = 'D:\\3 -IMOVEIS\\CONSTRUTORAS\\ATUAIS\\RL COSTA';
...
const EMPS = [
  {
    nome: 'ORO Conceito',
    dir:  path.join(BASE, '2026-08-20- ORO Conceito'),
    facade: null,
    body: bd('ORO Conceito, RL Costa em BH. Apartamentos de alto padrão entregues. Unidades esgotadas.','Belo Horizonte','30000-000'),
  },
  {
    nome: 'Safira RL Costa',
    dir:  path.join(BASE, '2026-08-20-Safira'),
    facade: null,
    body: { tipo:'apartamento', status:'pronto', descricao:'Safira, RL Costa em BH. Apartamentos com área privativa entregues. Unidades esgotadas.',
  ...
  {
    nome: 'Vetro',
    dir:  path.join(BASE, '2026-08-20-Vetro'),
    facade: '23005_RLC_Alorna179_02_Fachada_Noturna_R02_alta.jpeg',
    body: { tipo:'apartamento', status:'pronto', descricao:'Vetro, RL Costa na Rua Alorna, 179, BH. Apartamentos de alto padrão entregues com fachada noturna. Unidades esgotadas.',
      endereco:'Rua Alorna, 179', bairro:'Belo Horizonte', cidade:'Belo Horizonte', estado:'MG', cep:'30000-000',
      area_min:50, area_max:250, preco_min:null, preco_max:null, quartos_min:2, quartos_max:4, vagas:2 },
  },
  {
    nome: 'Zigra',
    dir:  path.join(BASE, '2026-08-20-Zigra'),
    facade: '24034_RLC_Zigra_01_Fachada_R02_alta.jpeg',
    body: bd('Zigra, RL Costa em BH. Apartamentos de alto padrão entregues. Unidades esgotadas.','Belo Horizonte','30000-000'),
  },
];

async function main() {
  console.log('═══════════════════════════════════════════════════');
  console.log('  RL COSTA — 4 empreendimentos');
  console.log('═══════════════════════════════════════════════════\n');
  let TOKEN;
  ...
  if (login.data?.access_token) { TOKEN = login.data.access_token; console.log('✅ Login OK'); }
    ...
    const reg = await api('/auth/register', { method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ email:EMAIL, password:SENHA, nome:'RL Costa', razao_social:'RL Costa Empreendimentos', role:'construtora' }) });
