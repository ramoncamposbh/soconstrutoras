/**
 * cadastrar-modernizar.js — Modernizar (1 empreendimento)
 *   Zalfa Berquó — Belo Horizonte (vendido)
 */
const fs   = require('fs');
const path = require('path');

const API   = 'https://soconstrutoras-production.up.railway.app/api/v1';
const EMAIL = 'modernizar@soconstrutoras.com.br';
const SENHA = 'MODERNIZAR@2026';
const BASE  = 'D:\\3 -IMOVEIS\\CONSTRUTORAS\\ATUAIS\\Modernizar';
...
const EMPS = [
  {
    nome: 'Zalfa Berquó',
    dir:  path.join(BASE, '2026-08-19-Zalfa Berquó'),
    facade: 'Apartamento_TIPO_FINAL_01.jpeg',
    body: {
      tipo:'apartamento', status:'pronto',
      descricao:'Zalfa Berquó, empreendimento Modernizar em Belo Horizonte. Apartamentos tipo e cobertura de alto padrão com acabamento diferenciado. Unidades esgotadas.',
...
async function main() {
  console.log('═══════════════════════════════════════════════════');
  console.log('  Modernizar — 1 empreendimento');
  console.log('═══════════════════════════════════════════════════\n');
  let TOKEN;
  ...
  if (login.data?.access_token) { TOKEN = login.data.access_token; console.log('✅ Login OK'); }
    ...
    const reg = await api('/auth/register', { method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ email:EMAIL, password:SENHA, nome:'Modernizar', razao_social:'Modernizar Empreendimentos Imobiliários', role:'construtora' }) });
