/**
 * cadastrar-soledificacoes.js — SOL EDIFICAÇÕES (2 empreendimentos — todos vendidos)
 */
const fs   = require('fs');
const path = require('path');

const API   = 'https://soconstrutoras-production.up.railway.app/api/v1';
const EMAIL = 'soledificacoes@soconstrutoras.com.br';
const SENHA = 'SOLEDIFICACOES@2026';
const BASE  = 'D:\\3 -IMOVEIS\\CONSTRUTORAS\\ATUAIS\\SOL EDIFICAÇÕES';
...
const EMPS = [
  {
    nome: 'Opus Residence',
    dir:  path.join(BASE, '2026-08-20-Opus Residence'),
    facade: null,
    body: { tipo:'apartamento', status:'pronto', descricao:'Opus Residence, SOL EDIFICAÇÕES em BH. Apartamentos de alto padrão entregues. Unidades esgotadas.',
  ...
  {
    nome: 'Soul Design Residence',
    dir:  path.join(BASE, '2026-08-20-Soul Design Residence'),
    facade: null,
    body: { tipo:'apartamento', status:'pronto', descricao:'Soul Design Residence, SOL EDIFICAÇÕES em BH. Apartamentos com área privativa e design diferenciado entregues. Unidades esgotadas.',
      endereco:'Belo Horizonte', bairro:'Belo Horizonte', cidade:'Belo Horizonte', estado:'MG', cep:'30000-000',
      area_min:50, area_max:250, preco_min:null, preco_max:null, quartos_min:2, quartos_max:4, vagas:2 },
  },
];
...
async function main() {
  console.log('═══════════════════════════════════════════════════');
  console.log('  SOL EDIFICAÇÕES — 2 empreendimentos');
  console.log('═══════════════════════════════════════════════════\n');
  let TOKEN;
  ...
  if (login.data?.access_token) { TOKEN = login.data.access_token; console.log('✅ Login OK'); }
    ...
    const reg = await api('/auth/register', { method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ email:EMAIL, password:SENHA, nome:'SOL EDIFICAÇÕES', razao_social:'SOL EDIFICAÇÕES Incorporações', role:'construtora' }) });
