/**
 * cadastrar-orkhestra.js — ORKHESTRA (3 empreendimentos)
 *   Allegro Residence, Legato, Residencial Harmonia — BH (vendidos)
 */
const fs   = require('fs');
const path = require('path');

const API   = 'https://soconstrutoras-production.up.railway.app/api/v1';
const EMAIL = 'orkhestra@soconstrutoras.com.br';
const SENHA = 'ORKHESTRA@2026';
const BASE  = 'D:\\3 -IMOVEIS\\CONSTRUTORAS\\ATUAIS\\ORKHESTRA';
...
const EMPS = [
  {
    nome: 'Allegro Residence',
    dir:  path.join(BASE, '2026-08-19-Allegro Residence'),
    facade: 'Imagem_do_WhatsApp_de_2025-04-22_a(s)_155544_0c28ad15.jpeg',
    body: {
      tipo:'apartamento', status:'pronto',
      descricao:'Allegro Residence, empreendimento ORKHESTRA em Belo Horizonte. Apartamentos de alto padrão entregues com lazer exclusivo. Unidades esgotadas.',
  ...
  {
    nome: 'Legato',
    dir:  path.join(BASE, '2026-08-19-Legato'),
    facade: '1001.jpeg',
    body: {
      tipo:'apartamento', status:'pronto',
      descricao:'Legato, empreendimento ORKHESTRA em Belo Horizonte. Apartamentos de alto padrão entregues. Unidades esgotadas.',
      endereco:'Belo Horizonte', bairro:'Belo Horizonte', cidade:'Belo Horizonte', estado:'MG', cep:'30000-000',
      area_min:70, area_max:300, preco_min:null, preco_max:null, quartos_min:2, quartos_max:4, vagas:2,
    },
  },
  {
    nome: 'Residencial Harmonia',
    dir:  path.join(BASE, '2026-08-19-Residencial Harmonia'),
    facade: 'Fachada_Harmonia.jpeg',
    body: {
      tipo:'apartamento', status:'pronto',
      descricao:'Residencial Harmonia, empreendimento ORKHESTRA em Belo Horizonte. Apartamentos de alto padrão entregues com lazer completo. Unidades esgotadas.',
      endereco:'Belo Horizonte', bairro:'Belo Horizonte', cidade:'Belo Horizonte', estado:'MG', cep:'30000-000',
      area_min:70, area_max:300, preco_min:null, preco_max:null, quartos_min:2, quartos_max:4, vagas:2,
    },
  },
...
async function main() {
  console.log('═══════════════════════════════════════════════════');
  console.log('  ORKHESTRA — 3 empreendimentos');
  console.log('═══════════════════════════════════════════════════\n');
  let TOKEN;
  ...
  if (login.data?.access_token) { TOKEN = login.data.access_token; console.log('✅ Login OK'); }
    ...
    const reg = await api('/auth/register', { method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ email:EMAIL, password:SENHA, nome:'ORKHESTRA', razao_social:'ORKHESTRA Empreendimentos Imobiliários', role:'construtora' }) });
