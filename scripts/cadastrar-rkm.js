/**
 * cadastrar-rkm.js — RKM (2 empreendimentos — todos vendidos)
 */
const fs   = require('fs');
const path = require('path');

const API   = 'https://soconstrutoras-production.up.railway.app/api/v1';
const EMAIL = 'rkm@soconstrutoras.com.br';
const SENHA = 'RKM@2026';
const BASE  = 'D:\\3 -IMOVEIS\\CONSTRUTORAS\\ATUAIS\\RKM';
...
const EMPS = [
  {
    nome: 'Float São Paulo',
    dir:  path.join(BASE, 'FLOAT SAO PAULO'),
    facade: null,
    body: { tipo:'apartamento', status:'pronto', descricao:'Float São Paulo, RKM em São Paulo. Apartamentos de alto padrão entregues. Unidades esgotadas.',
  ...
  {
    nome: 'Sereno - Orvalho T1',
    dir:  path.join(BASE, 'Sereno - Orvalho (T1)'),
    facade: null,
    body: { tipo:'apartamento', status:'pronto', descricao:'Sereno Orvalho Torre 1, RKM em BH. Apartamentos de alto padrão entregues. Unidades esgotadas.',
...
async function main() {
  console.log('═══════════════════════════════════════════════════');
  console.log('  RKM — 2 empreendimentos');
  console.log('═══════════════════════════════════════════════════\n');
  let TOKEN;
  ...
  if (login.data?.access_token) { TOKEN = login.data.access_token; console.log('✅ Login OK'); }
    ...
    const reg = await api('/auth/register', { method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ email:EMAIL, password:SENHA, nome:'RKM', razao_social:'RKM Incorporações e Construções', role:'construtora' }) });
