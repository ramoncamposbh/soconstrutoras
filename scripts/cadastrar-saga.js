/**
 * cadastrar-saga.js — SAGA (3 empreendimentos — todos vendidos)
 */
const fs   = require('fs');
const path = require('path');

const API   = 'https://soconstrutoras-production.up.railway.app/api/v1';
const EMAIL = 'saga@soconstrutoras.com.br';
const SENHA = 'SAGA@2026';
const BASE  = 'D:\\3 -IMOVEIS\\CONSTRUTORAS\\ATUAIS\\SAGA';
...
const EMPS = [
  {
    nome: 'Easy Residence',
    dir:  path.join(BASE, '2026-08-20 EASY RESIDENCE'),
    facade: null,
    body: bd('Easy Residence, SAGA em BH. Apartamentos entregues. Unidades esgotadas.','Belo Horizonte','30000-000',1,3),
  },
  {
    nome: 'Brisa Residencial',
    dir:  path.join(BASE, '2026-08-20- BRISA'),
    facade: null,
    body: bd('Brisa Residencial, SAGA em BH. Apartamentos de alto padrão entregues. Unidades esgotadas.','Belo Horizonte','30000-000'),
  },
  {
    nome: 'Terrá Luxemburgo',
    dir:  path.join(BASE, '2026-08-20-TERRÁ LUXEMBURGO'),
    facade: 'P01_FachadaFrontal.jpeg',
    body: { tipo:'apartamento', status:'pronto', descricao:'Terrá Luxemburgo, SAGA em BH. Apartamentos de alto padrão entregues com fachada frontal. Unidades esgotadas.',
      endereco:'Belo Horizonte', bairro:'Luxemburgo', cidade:'Belo Horizonte', estado:'MG', cep:'30380-000',
...
async function main() {
  console.log('═══════════════════════════════════════════════════');
  console.log('  SAGA — 3 empreendimentos');
  console.log('═══════════════════════════════════════════════════\n');
  let TOKEN;
  ...
  if (login.data?.access_token) { TOKEN = login.data.access_token; console.log('✅ Login OK'); }
    ...
    const reg = await api('/auth/register', { method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ email:EMAIL, password:SENHA, nome:'SAGA', razao_social:'SAGA Empreendimentos Imobiliários', role:'construtora' }) });
