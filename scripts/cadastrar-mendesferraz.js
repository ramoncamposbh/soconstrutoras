/**
 * cadastrar-mendesferraz.js — MENDES FERRAZ (2 empreendimentos)
 *   ED. SOLARIUM — BH (vendido)
 *   Ed. Nice     — BH (vendido)
 */
const fs   = require('fs');
...
const EMAIL = 'mendesferraz@soconstrutoras.com.br';
const SENHA = 'MENDESFERRAZ@2026';
const BASE  = 'D:\\3 -IMOVEIS\\CONSTRUTORAS\\ATUAIS\\MENDES FERRAZ';
...
const EMPS = [
  {
    nome: 'ED. SOLARIUM',
    dir:  path.join(BASE, '2026-08-19-ED. SOLARIUM'),
    facade: 'SOLARIUM_REVIT_3(2).jpeg',
    body: {
      tipo:'apartamento', status:'pronto',
      descricao:'Edifício Solarium, empreendimento Mendes Ferraz em Belo Horizonte. Apartamentos de alto padrão entregues. Unidades esgotadas.',
  ...
  {
    nome: 'Ed. Nice',
    dir:  path.join(BASE, '2026-08-19-Ed. Nice'),
    facade: 'ED_NICE_-_Fachada.jpeg',
    body: {
      tipo:'apartamento', status:'pronto',
      descricao:'Edifício Nice, empreendimento Mendes Ferraz em Belo Horizonte. Apartamentos de alto padrão entregues com acabamento superior. Unidades esgotadas.',
      endereco:'Belo Horizonte', bairro:'Belo Horizonte', cidade:'Belo Horizonte', estado:'MG', cep:'30000-000',
      area_min:70, area_max:250, preco_min:null, preco_max:null, quartos_min:2, quartos_max:4, vagas:2,
    },
  },
...
async function main() {
  console.log('═══════════════════════════════════════════════════');
  console.log('  MENDES FERRAZ — 2 empreendimentos');
  console.log('═══════════════════════════════════════════════════\n');
  let TOKEN;
  ...
  if (login.data?.access_token) { TOKEN = login.data.access_token; console.log('✅ Login OK'); }
    ...
    const reg = await api('/auth/register', { method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ email:EMAIL, password:SENHA, nome:'Mendes Ferraz', razao_social:'Mendes Ferraz Empreendimentos Imobiliários', role:'construtora' }) });
