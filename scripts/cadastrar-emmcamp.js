...
const EMAIL = 'emmcamp@soconstrutoras.com.br';
const SENHA = 'EMMCAMP@2026';
const BASE  = 'D:\\3 -IMOVEIS\\CONSTRUTORAS\\ATUAIS\\emmcamp';
...
const EMPS = [
  { nome:'Brizze',
    dir: BASE,
    facade: null,
    body: { tipo:'apartamento', status:'pronto', descricao:'Brizze, emmcamp em BH. Apartamentos de 2 quartos entregues. Unidades esgotadas.',
      endereco:'Belo Horizonte', bairro:'Belo Horizonte', cidade:'Belo Horizonte', estado:'MG', cep:'30000-000',
      area_min:50, area_max:150, preco_min:null, preco_max:null, quartos_min:2, quartos_max:2, vagas:1 } },
];
...
async function main() {
  ...
  if (login.data?.access_token) { TOKEN = login.data.access_token; console.log('✅ Login OK'); }
    ...
    const reg = await api('/auth/register', { method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ email:EMAIL, password:SENHA, nome:'emmcamp', razao_social:'emmcamp Incorporadora', role:'construtora' }) });
