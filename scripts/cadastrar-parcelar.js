/**
 * cadastrar-parcelar.js — PARCELAR (1 empreendimento)
 *   Monjardim — Belo Horizonte (vendido)
 */
const fs   = require('fs');
const path = require('path');

const API   = 'https://soconstrutoras-production.up.railway.app/api/v1';
const EMAIL = 'parcelar@soconstrutoras.com.br';
const SENHA = 'PARCELAR@2026';
const BASE  = 'D:\\3 -IMOVEIS\\CONSTRUTORAS\\ATUAIS\\PARCELAR';
...
const EMPS = [
  {
    nome: 'Monjardim',
    dir:  path.join(BASE, '2026-08-19-Monjardim'),
    facade: 'Captura_de_tela_2025-10-28_145508.jpeg',
    body: {
      tipo:'apartamento', status:'pronto',
      descricao:'Monjardim, empreendimento PARCELAR em Belo Horizonte. Apartamentos de alto padrão com lazer completo. Unidades esgotadas.',
...
async function main() {
  console.log('═══════════════════════════════════════════════════');
  console.log('  PARCELAR — 1 empreendimento');
  console.log('═══════════════════════════════════════════════════\n');
  let TOKEN;
  ...
  if (login.data?.access_token) { TOKEN = login.data.access_token; console.log('✅ Login OK'); }
    ...
    const reg = await api('/auth/register', { method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ email:EMAIL, password:SENHA, nome:'PARCELAR', razao_social:'PARCELAR Empreendimentos Imobiliários', role:'construtora' }) });
