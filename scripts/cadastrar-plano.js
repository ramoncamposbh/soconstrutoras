/**
 * cadastrar-plano.js — PLANO (1 empreendimento: Toscana)
 */
const fs   = require('fs');
const path = require('path');
const API   = 'https://soconstrutoras-production.up.railway.app/api/v1';
const EMAIL = 'plano@soconstrutoras.com.br';
const SENHA = 'PLANO@2026';
const BASE  = 'D:\\3 -IMOVEIS\\CONSTRUTORAS\\ATUAIS\\PLANO';
...
const EMPS = [
  { nome:'Toscana PLANO',
    dir: BASE,
    facade: '8_-_FACHADA_RUA_ROSINHA_SIGAUD.jpeg',
    body: { tipo:'apartamento', status:'pronto',
      descricao:'Toscana, PLANO em BH. Apartamentos de alto padrão com área privativa, piscina e salão de festas entregues. Unidades esgotadas.',
      endereco:'Rua Rosinha Sigaud', bairro:'Belo Horizonte', cidade:'Belo Horizonte', estado:'MG', cep:'30000-000',
...
async function main() {
  ...
  if (login.data?.access_token) { TOKEN = login.data.access_token; console.log('✅ Login OK'); }
    ...
    const reg = await api('/auth/register', { method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ email:EMAIL, password:SENHA, nome:'PLANO', razao_social:'PLANO Incorporadora', role:'construtora' }) });
